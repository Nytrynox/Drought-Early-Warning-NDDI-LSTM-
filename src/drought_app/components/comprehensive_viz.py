...
def create_3d_terrain_map(df, lat_col='Latitude', lon_col='Longitude', value_col='NDDI'):
    """
    Create 3D terrain-style map
    """
    
    if all(col in df.columns for col in [lat_col, lon_col, value_col]):
        # Create grid
        lat_unique = np.linspace(df[lat_col].min(), df[lat_col].max(), 50)
        lon_unique = np.linspace(df[lon_col].min(), df[lon_col].max(), 50)
        ...
        value_grid = griddata(points, values, (lat_grid, lon_grid), method='cubic')
